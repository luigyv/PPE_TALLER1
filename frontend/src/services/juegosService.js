...
// Mientras A no avise que el backend está listo: VITE_USE_MOCK=true
// Cuando A avise: cambia VITE_USE_MOCK=false en el .env
  ...
  const filtered = term
    ? list.filter(
        (j) =>
          ...
          j.categoria.toLowerCase().includes(term)
      )
    : list
...
export async function listarJuegos({
  ...
  if (USE_MOCK) {
    ...
    return paginateAndFilter(_getAll(), {
      search,
      page,
      limit,
    })
  ...
  // Backend real
  const { data } = await http.get('/juegos', {
    params: {
      search,
      page,
      limit,
  ...
  // Adaptamos la respuesta del backend al formato
